...
class MyApp
{
    ...
    function authorizedUDI($roles, $udi)
    {
        ...
        if (isset($controller_acl[$action_name]))
        {
            ...
            return Admin::has_perm($controller_acl[$action_name]);
        }

        if (isset($controller_acl['_allow']))
        {
            // 如果为所有动作指定了默认 ACT，则使用该 ACT 进行检查
            return Admin::has_perm($controller_acl['_allow']);
        }
...
$_MQ_ = get_magic_quotes_gpc();
function s($str) {
	global $_MQ_;
    if ($_MQ_ && QContext::instance()->isPOST()) return $str;
	$str = str_replace('\\', '\\\\', $str);
