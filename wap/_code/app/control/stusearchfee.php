...
class Control_StuSearchfee extends QUI_Control_Abstract
{
    function render()
    {
        foreach ($_GET as $k=>$v) {
        ...
        $out = $this->_fetchView(dirname(__FILE__) . '/stusearchfee_view');
    ...
    public static function filterCond($sql_where, $tbl='') {
        ...
        if($qenroll!=null&&$qenroll!=""){
            $sql_where .= " and [{$tbl}enroll_id] in (".$qenroll.")";
        }
        if($qorgedu!=null&&$qorgedu!=""){
            $sql_where .= " and [{$tbl}college_id] in (".$qorgedu.")";
        }
        if($qorglen!=null&&$qorglen!=""){
            $sql_where .= " and [{$tbl}training_id] in (".$qorglen.")";
        }
        if($qdiscipline!=null&&$qdiscipline!=""){
            $sql_where .= " and [{$tbl}discipline_id] in (".$qdiscipline.")";
        }
        if($qclassinfo!=null&&$qclassinfo!=""){
            $sql_where .= " and [{$tbl}classid] in (".$qclassinfo.")";
        }
        if($qname!=null&&$qname!=""){
