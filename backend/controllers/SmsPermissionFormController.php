...
use Yii;
use backend\models\SmsPermissionForm;
use backend\models\SmsPermissionFormSearch;
use backend\models\SmsMenu;
 ...
 */
class SmsPermissionFormController extends CController
{
    private $menuList = [];
    public function init(){
        parent::init();
        $this->subject = '站点管理系统权限管理';
        $menuModel = new SmsMenu();
    ...
    public function actionIndex()
    {
        ...
        $searchModel = new SmsPermissionFormSearch();
    ...
    {
        $model = new SmsPermissionForm();
        $post = Yii::$app->request->post();
        unset($this -> menuList[0]);
        if ($model->load($post) && $model->save()) {
            $model->menu_id = $post['SmsPermissionForm']['menu_id'];
            $model->save();
    ...
    public function actionDelete($id)
    {
        ...
        if($params['id']){
            SmsPermissionForm::deleteAll(['name' => $params['id']]);
        }
        return $this->render('create');
    ...
    {
        if (($model = SmsPermissionForm::findOne($id)) !== null) {
            return $model;
        } else {
